@echo off
echo ===== Deploying Virtual Lawyer to Heroku (Simple Method) =====
echo.
...
echo.
echo Step 3: Building the application for deployment...
call npm run build:frontend

echo.
echo Step 4: Creating a temporary directory for deployment...
mkdir heroku-deploy
cd heroku-deploy

echo.
echo Step 5: Initializing a new Git repository...
git init

echo.
echo Step 6: Creating necessary files for Heroku...
echo web: node ../server-combined.js > Procfile
echo { > package.json
echo   "name": "virtual-lawyer", >> package.json
echo   "version": "1.0.0", >> package.json
echo   "engines": { >> package.json
echo     "node": "14.x" >> package.json
echo   }, >> package.json
echo   "scripts": { >> package.json
echo     "start": "node ../server-combined.js" >> package.json
echo   } >> package.json
echo } >> package.json

echo.
echo Step 7: Creating or updating the Heroku app...
...
echo.
echo Step 9: Setting up MongoDB...
...
echo.
echo Step 10: Deploying to Heroku...
git add .
git commit -m "Deploy to Heroku"
git push heroku master

echo.
echo Step 11: Ensuring at least one instance is running...
heroku ps:scale web=1 --app %APP_NAME%

echo.
echo Step 12: Cleaning up...
cd ..
rmdir /s /q heroku-deploy
